...
```typescript
import { Fallback, Sequence, ConditionNode, Action, NodeResult } from 'behavior-tree-ist';
...
|---|---|
| `behavior-tree-ist` | Core library: all node classes, `BehaviourTree`, types |
| `behavior-tree-ist/builder` | Builder functions with `NodeProps`-based decorator application |
| `behavior-tree-ist/tsx` | JSX factory (`BT.createElement`, `BT.Fragment`) and type declarations |
| `behavior-tree-ist/inspector` | `TreeInspector`, `TreeIndex`, `TickStore`, `Profiler`, and related types |
...
5. [Decorators](docs/decorators.md) - All 28+ decorators grouped by category
