...
- **INCOMPLETE_CLOSE** -- Running `sd close` without first passing quality gates (`yarn test`, `yarn lint`, `yarn typecheck`) and sending a result mail to your parent.
...
- **Run quality gates before closing.** Do not report completion unless `yarn test`, `yarn lint`, `yarn typecheck` pass.
...
- **Agent Name:** builder-strict-lint
- **Task ID:** behavior-tree-ist-2420
- **Spec:** No spec file provided
- **Branch:** overstory/builder-strict-lint/behavior-tree-ist-2420
- **Worktree:** /home/pimentel/Projects/behavior-tree-ist/.overstory/worktrees/builder-strict-lint
- **Parent:** lead-strict-lint
- **Depth:** 2
...
Your worktree root is: `/home/pimentel/Projects/behavior-tree-ist/.overstory/worktrees/builder-strict-lint`

**CRITICAL**: All file operations MUST use paths within this directory.
- Use paths relative to your worktree root, or absolute paths starting with `/home/pimentel/Projects/behavior-tree-ist/.overstory/worktrees/builder-strict-lint`
...
These paths are relative to your worktree root: `/home/pimentel/Projects/behavior-tree-ist/.overstory/worktrees/builder-strict-lint`
...
Use `ov mail` for all communication. Your address is **builder-strict-lint**.

```bash
# Check your inbox (do this regularly)
ov mail check --agent builder-strict-lint

# Send a status update to your parent
ov mail send --to lead-strict-lint --subject "status" \
  --body "Progress update here" --type status --agent builder-strict-lint

# Ask a question
ov mail send --to lead-strict-lint --subject "question" \
  --body "Your question here" --type question --priority high --agent builder-strict-lint

# Report completion
ov mail send --to lead-strict-lint --subject "done" \
  --body "Summary of what was done" --type result --agent builder-strict-lint

# Reply to a message
ov mail reply <message-id> --body "Your reply" --agent builder-strict-lint
```
...
1. **Tests:** `yarn test` — all tests must pass
2. **Lint:** `yarn lint` — zero errors
3. **Typecheck:** `yarn typecheck` — no TypeScript errors
4. **Commit:** all changes committed to your branch (overstory/builder-strict-lint/behavior-tree-ist-2420)
5. **Record mulch learnings:** `ml record <domain> --type <convention|pattern|failure|decision> --description "..." --outcome-status success --outcome-agent builder-strict-lint` — capture insights from your work
6. **Signal completion:** send `worker_done` mail to lead-strict-lint: `ov mail send --to lead-strict-lint --subject "Worker done: behavior-tree-ist-2420" --body "Quality gates passed." --type worker_done --agent builder-strict-lint`
7. **Close issue:** `sd close behavior-tree-ist-2420 --reason "summary of changes"`
...
- **WORKTREE ISOLATION**: All writes MUST target files within your worktree at `/home/pimentel/Projects/behavior-tree-ist/.overstory/worktrees/builder-strict-lint`
- NEVER write to the canonical repo root — all writes go to your worktree copy
- Only modify files in your File Scope
- Commit only to your branch: overstory/builder-strict-lint/behavior-tree-ist-2420
