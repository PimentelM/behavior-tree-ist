...
**behavior-tree-ist** — a code-first TypeScript behaviour tree library focused on developer experience. Provides composable, type-safe nodes with lifecycle hooks, decorators, builder functions, JSX support, and runtime inspection. Structured as a monorepo with the core library at `packages/core/`.

## Commands

```bash
yarn test              # Run all tests (vitest workspace)
yarn test:watch        # Run tests in watch mode
yarn test packages/core/src/nodes/decorators/repeat.test.ts  # Run a single test file
...
- **Composite nodes** (packages/core/src/nodes/composite/): Have multiple children — `Sequence` (AND), `Fallback` (OR), `Parallel`, memory variants, `IfThenElse`, utility-scored variants
- **Decorator nodes** (packages/core/src/nodes/decorators/): Wrap a single child — timing guards, result transformers, control flow, lifecycle hooks
...
Three-valued: `Succeeded`, `Failed`, `Running`. Defined as a const object + type in packages/core/src/base/types.ts.
...
2. **Builder functions** (@behavior-tree-ist/core/builder) — `sequence()`, `action()`, `condition()`, etc. Accept props that auto-apply decorators
3. **JSX/TSX** (@behavior-tree-ist/core/tsx) — `<sequence>`, `<action>` etc. with decorator props; uses custom factory `BT.createElement`
...
- **Test helpers** (packages/core/src/test-helpers.ts): prefer `createNodeTicker()` over manually creating a TickContext — it handles tick ID incrementing and provides both `tick()` and `abort()` methods. Use `StubAction` for configurable result queues with lifecycle counters.
...
|---|---|
| `packages/core/src/base/node.ts`, `packages/core/src/base/types.ts` | `docs/core-concepts.md`, `docs/node-flags.md` |
| `packages/core/src/base/action.ts`, `packages/core/src/base/condition.ts`, `packages/core/src/nodes/actions/` | `docs/leaf-nodes.md` |
| `packages/core/src/nodes/composite/` | `docs/composite-nodes.md` |
| `packages/core/src/nodes/decorators/` | `docs/decorators.md` |
| `packages/core/src/builder/index.ts` (NodeProps, builder functions) | `docs/construction-apis.md` |
| `packages/core/src/tsx/` | `docs/tsx.md` |
| `packages/core/src/inspector/` | `docs/inspector.md` |
