...
describe("TickStore", () => {
    ...
    describe("insertBefore", () => {
        it("prepends older records before current window", () => {
            const store = new TickStore(5);
            store.push(makeRecord(10));
            store.push(makeRecord(11));

            const evicted = store.insertBefore([makeRecord(7), makeRecord(8), makeRecord(9)]);

            expect(evicted).toEqual([]);
            expect(store.getStoredTickIds()).toEqual([7, 8, 9, 10, 11]);
            expect(store.hasTick(7)).toBe(true);
            expect(store.hasTick(11)).toBe(true);
        });

        it("evicts from newest end when overflow", () => {
            const store = new TickStore(4);
            store.push(makeRecord(10));
            store.push(makeRecord(11));
            store.push(makeRecord(12));

            const evicted = store.insertBefore([makeRecord(7), makeRecord(8), makeRecord(9)]);

            expect(evicted.map(e => e.tickId)).toContain(12);
            expect(store.getStoredTickIds()).toEqual([7, 8, 9, 10]);
            expect(store.hasTick(12)).toBe(false);
            expect(store.hasTick(7)).toBe(true);
        });

        it("filters records >= current oldest", () => {
            const store = new TickStore(10);
            store.push(makeRecord(10));

            const evicted = store.insertBefore([makeRecord(10), makeRecord(11), makeRecord(9)]);

            expect(evicted).toEqual([]);
            expect(store.getStoredTickIds()).toEqual([9, 10]);
        });

        it("filters empty-event records", () => {
            const store = new TickStore(10);
            store.push(makeRecord(10));

            store.insertBefore([{ tickId: 5, timestamp: 5000, events: [], refEvents: [] }]);

            expect(store.getStoredTickIds()).toEqual([10]);
        });

        it("filters duplicates within batch and existing store", () => {
            const store = new TickStore(10);
            store.push(makeRecord(5));
            store.push(makeRecord(10));

            store.insertBefore([makeRecord(3), makeRecord(3), makeRecord(5)]);

            expect(store.getStoredTickIds()).toEqual([3, 5, 10]);
            expect(store.hasTick(3)).toBe(true);
        });
        ...
        it("sorts unsorted input correctly", () => {
            ...
            store.insertBefore([makeRecord(9), makeRecord(7), makeRecord(8)]);
        ...
        it("returns empty for empty input", () => {
            ...
            expect(store.insertBefore([])).toEqual([]);
            expect(store.getStoredTickIds()).toEqual([10]);
        });

        it("keeps byTickId in sync after eviction", () => {
            const store = new TickStore(3);
            store.push(makeRecord(10));
            store.push(makeRecord(11));
            store.push(makeRecord(12));

            store.insertBefore([makeRecord(7), makeRecord(8), makeRecord(9)]);

            expect(store.hasTick(10)).toBe(false);
            expect(store.hasTick(11)).toBe(false);
            expect(store.hasTick(12)).toBe(false);
            expect(store.getByTickId(7)).toBeDefined();
            expect(store.getByTickId(8)).toBeDefined();
            expect(store.getByTickId(9)).toBeDefined();
        });

        it("works with empty store (no current oldest)", () => {
            const store = new TickStore(5);

            store.insertBefore([makeRecord(3), makeRecord(5)]);

            expect(store.getStoredTickIds()).toEqual([3, 5]);
        });
    });
