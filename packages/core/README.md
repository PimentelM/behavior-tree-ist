...
- **Ref system**: `Ref<T>`, `ReadonlyRef<T>`, `DerivedRef<T>`, `multiRef()`, and `patchRef()` for type-safe, auto-traced state sharing between nodes. Refs with dot-notation names (e.g. `multiRef("combat", { target, weapon })` producing `combat.target`, `combat.weapon`) are displayed as collapsible groups in the Studio UI.
